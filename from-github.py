...
def select_action(state):
    ...
    if sample > eps_threshold:
        # why? https://stackoverflow.com/a/72504818
        # when we're updating weights, you  DON'T want pytorch to recalculate the gradient, because we're not
        # not just updating weights but some other situations where you don't want the whole thing to change while you're still modifying it
        # finished with updating the whole network yet, so it'll mess up.
        with torch.no_grad():
            # t.max(1) will return the largest column value of each row.
            # second column on max result is index of where max element was
            # found, so we pick action with the larger expected reward.
            
            # this part isn't too hard btw, it's just running the network and getting the idx of highest output
            # I think when you call NN.module(state) that's literally feeding an input state and getting the results
            # very subtle huh?
            # # print(1, policy_net(state))
            # get largest output
            # print(2, policy_net(state).max(1))
            # get its index
            # print(3, policy_net(state).max(1)[1])
            # Tensor.view(1, 1) turns it into a 2D array with 1 col and 1 row
            # print(3, policy_net(state).max(1)[1].view(1, 1))
...
# added stability. The targetet network is updated at every step with a 
